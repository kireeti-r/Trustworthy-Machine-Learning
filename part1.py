...
#### Put your code here
def laplace_mechanism(pred_y, epsilon):
    sensitivity = np.max(np.abs(pred_y - np.roll(pred_y, 1)))
    beta = sensitivity / epsilon
    noise = np.random.laplace(scale=beta, size=pred_y.shape)
    return pred_y + noise

def dp_predict(model, x, epsilon=1.0):
    pred_y = basic_predict(model, x)
    pred_y_dp = laplace_mechanism(pred_y, epsilon)
    return pred_y_dp
...
# def fgsa_attack(model, x, y, eps, alpha, iters):
#     """
...
#         with torch.no_grad():
#             grad = alpha * x.grad.sign()
#             x_perturbed = x + grad
#             x_perturbed = torch.max(torch.min(x_perturbed, x + eps), x - eps) # clip to eps range
#             x_perturbed = torch.clamp(x_perturbed, 0, 1) # clamp to [0,1] range
        
#         # Update x for next iteration
#         x = x_perturbed.detach().clone().requires_grad_()
    
#     # Return perturbed inputs as numpy array
#     return x.cpu().numpy()
...
if __name__ == "__main__":
    ...
    predict_fn = lambda x: dp_predict(model, x, epsilon=1.0)
